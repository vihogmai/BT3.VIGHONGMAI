...
### 1. Khách hàng & Hợp đồng (Mối quan hệ 1-N)
* **Đoạn thể hiện:** Ràng buộc `FK_HopDong_KhachHang` trong bảng `HopDong`.
* **Ý nghĩa:** Một khách hàng có thể có nhiều hợp đồng cầm cố khác nhau. Khóa ngoại `MaKH` trong bảng `HopDong` đảm bảo mọi hợp đồng đều thuộc về một khách hàng cụ thể và cho phép một khách hàng mở nhiều khoản vay theo thời gian.
  ### 2. Hợp đồng & Tài sản (Mối quan hệ 1-N)
* **Đoạn thể hiện:** Ràng buộc `FK_TaiSan_HopDong` trong bảng `TaiSan`.
* **Ý nghĩa:** Một hợp đồng có thể bao gồm nhiều tài sản thế chấp khác nhau. Điều này cho phép khách hàng dùng nhiều món đồ (ví dụ: xe máy và điện thoại) để đảm bảo cho cùng một khoản vay. Tất cả tài sản này đều liên kết với một mã hợp đồng (`MaHD`) duy nhất, hỗ trợ việc quản lý và trả đồ từng phần.
